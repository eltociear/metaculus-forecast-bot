...
FUNDED = [("minibench", "MiniBench", "$1,000"),
          ("animal", "Animal Futures", "$3,400"),
          ("pulse", "Market Pulse 26Q3", "$7,500"),
          ("summer", "Summer FutureEval", "$50,000"),
          ("cup", "Metaculus Cup", "no bot prize")]
...
def leaderboard_probe(tid) -> str:
    """Find any headless read of our standing. A 404 everywhere is itself the answer:
    it means standing is only visible in the UI and cannot drive automation."""
    for path in (f"/projects/{tid}/leaderboard/", f"/leaderboards/?project_id={tid}",
                 f"/tournaments/{tid}/leaderboard/"):
        try:
            d = _request(f"{API_BASE_URL}{path}", headers=metaculus_headers())
            entries = d.get("entries") or d.get("results") or []
            if entries:
                return f"{path} -> {len(entries)} entries"
            return f"{path} -> 200 but empty"
        except urllib.error.HTTPError as e:
            continue
        except Exception:  # noqa: BLE001
            continue
    return "no headless leaderboard endpoint (404 on all three)"
...
def main() -> int:
    ...
    for key, label, prize in FUNDED:
        ...
        print(f"  standing: {leaderboard_probe(tid)}")
